---
alwaysApply: false
description: 项目的样式规范，包括 SCSS Modules 使用与自定义主题 CSS 变量。当编写组件样式或主题适配时读取此规则。
---
...
- UI 交互保持 Radix + 自研组件体验一致

## 主题色 CSS 变量（NON-NEGOTIABLE）

在自定义样式的场景下，必须使用主题色 CSS 变量，**严格禁止**硬编码颜色值：

- **自定义变量**：`var(--shentu-card-bg)`、`var(--shentu-border-radius)`、`var(--shentu-cn-family)` 等
