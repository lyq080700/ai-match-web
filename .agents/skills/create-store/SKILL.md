...
- 为业务模块新增全局状态（如主题、用户信息）
- 重构原有的状态管理逻辑到统一的 `stores` 目录
...
- 所有 store 文件必须放在：`stores`
- 每个业务模块一个文件：`stores/<module>.ts`
...
export const useThemeStore = create<ThemeState>()(
  persist(
    ...
    {
      name: 'ai-match-theme', // localStorage key，建议带项目前缀
    }
  )
);
```
...
- 状态逻辑必须集中在 `stores`，**禁止** 在组件层直接用 `useState` 维护本应全局共享的业务状态。
