...
- **通用组件**（跨页面复用）：`components/<component-name>/`
- **页面级组件**（只在单页使用）：`app/<route>/components/`
...
```text
app/(main)/home/components/header/
  ├─ index.tsx
  └─ index.module.scss
```
...
```tsx
// components/shentu-button/index.tsx
...
```scss
// components/shentu-button/index.module.scss
.shentuButton {
  padding: 8px 16px;
  border-radius: 4px;
  border: none;
...
- 自定义颜色必须使用主题 CSS 变量：`var(--shentu-xxx)` 等
...
通用交互控件应基于 Radix UI 进行二次封装：

```tsx
import React from 'react';
import * as Button from '@radix-ui/react-button';

type ShentuButtonProps = Button.ButtonProps & {
  // 可以扩展一些自定义属性
};
...
- [ ] 是否通过 `components/index.ts` 集中导出通用组件？
